...
'''
away_teams = [event[:(event.find(" at ")-1)] for event in events]
'''
...
'''
home_teams = [event[(event.find(" at ")+4):(event.find(" Ticket")-1)]
'''
...
prices_home_ravens = []
for home_team, price in zipped:
    if home_team == "Baltimore Raven":
        prices_home_ravens.append(price)
'''
Step 7: Create a list of prices for Ravens away games.
'''
zipped = zip(away_teams, prices)
prices_away_ravens = []
for away_team, price in zipped:
    if away_team == "Baltimore Raven":
        prices_away_ravens.append(price)

'''
Step 8: Calculate the average of each of the price lists.
'''
average_ravens_home = sum(prices_home_ravens)/float(len(prices_home_ravens))
average_ravens_away = sum(prices_away_ravens)/float(len(prices_away_ravens))

print "Average Ravens home game ticket is ", average_ravens_home
print "Average Ravens away game ticket is ", average_ravens_away
print "Average NFL game ticket is", average_price
