...
if __name__ == '__main__':
    ...
    #response_columns = ['1_min_updown','5_min_updown','10_min_updown','20_min_updown','40_min_updown','80_min_updown'] #binary
    response_columns = ['1_min_cat','5_min_cat','10_min_cat','20_min_cat','40_min_cat','80_min_cat'] #categorical
    ...
    depth_list = range(8,21,2) #range(1,8) + range(8,21,2)
    ...
    for response in response_columns:
        for num_features in num_feature_list:
            ...
            for max_depth in depth_list:
                ...
                cross_val_score_list = cross_val_score(clf, X, y, cv=tscvsplit(one_minute_dataframe,CROSS_VALIDATION_FOLDS), n_jobs = -1)

                if response not in accuracy_results:
                    accuracy_results[response] = {num_features: {max_depth: np.mean(cross_val_score_list)}}
                else:
                    if num_features not in accuracy_results[response]:
                        accuracy_results[response][num_features] = {max_depth: np.mean(cross_val_score_list)}
                    else:
                        accuracy_results[response][num_features][max_depth] = np.mean(cross_val_score_list)
    
    print "response\tnum_features\tmax_depth\taccuracy"
    for response in response_columns:
        for num_features in num_feature_list:
            for max_depth in depth_list:
                print "%s\t%s\t\t%s\t\t%0.15f" % (response, num_features, max_depth, accuracy_results[response][num_features][max_depth])
